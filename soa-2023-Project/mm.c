...
struct pos_vector_mem{
  int id_frame_ph;
  int num_ref;
  int delete;
};

 struct pos_vector_mem vector_de_frames[10];


void init_vector_de_frames(){
  for(int i = 0; i<10; ++i){
    int id=alloc_frame();
    if(id>=0){
        vector_de_frames[i].id_frame_ph = id;
        vector_de_frames[i].num_ref = 0;
        vector_de_frames[i].delete = 0;
    }
  }
}